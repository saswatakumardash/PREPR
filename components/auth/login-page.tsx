...
// Dynamically import the 3D effect to avoid SSR issues
const Login3DEffect = dynamic(() => import('./login-3d-effect'), { ssr: false });
...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  return (
    <ThemeProvider defaultTheme="dark" storageKey="prepr-ui-theme">
      <div className="min-h-screen w-full flex flex-col items-stretch justify-center p-0 relative bg-white dark:bg-black transition-colors duration-200" style={{ fontFamily: 'Inter, system-ui, Segoe UI, sans-serif', fontSize: '1.08rem' }}>
        ...
        {/* Login Card Centered */}
        <div className="w-full flex flex-col items-center justify-center p-4 relative z-10 min-h-screen">
          {/* Theme Toggle & Clock */}
          ...
          <div className="relative z-10 w-full max-w-md mx-auto flex-1 flex flex-col justify-center items-center">
            <Card className="w-full shadow-none rounded-xl p-8 flex flex-col items-center gap-6 border transition-colors duration-200
              ...
              <div className="text-sm font-medium text-center mb-2 dark:text-white/60 text-gray-600">Ace Your Coding Interviews & Placements</div>
              {/* Typing Animation */}
